      ...
      <div class="col-12 xl:col-6"
        ...
        <div class="h-full w-full m-0 py-7 px-4"
          ...
          <div class="text-center mb-5">
            <img alt="Image" class="mb-3" height="50" src="layout/images/avatar.png">
          ...
          <div class="w-full md:w-10 mx-auto">
            ...
            <InputText id="email1" v-model="email" class="w-full mb-3" placeholder="Email" style="padding:1rem;"
                       type="text"/>

            <label class="block text-900 font-medium text-xl mb-2" for="password1">Password</label>
            <Password id="password1" v-model="password" :toggleMask="true" class="w-full mb-3"
                      inputClass="w-full" inputStyle="padding:1rem" placeholder="Password"></Password>
            ...
            <Button class="w-full p-3 text-xl" label="Sign In" @click="loginAction"></button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    loginAction() {
      ...
      const refresh = 'this_is_refresh'
      const userInfo = JSON.stringify({
        name: 'User name'
      })
      this.login({token, refresh})
