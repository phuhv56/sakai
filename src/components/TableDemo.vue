<template>
  <div class="grid">
    <div class="col-12">
      <div class="card">
        <h5>Filter Menu</h5>
        <DataTable v-model:filters="filters1" :filters="filters1" :globalFilterFields="['name','country.name','representative.name','balance','status']" :loading="loading1" :paginator="true"
                   :rowHover="true"
                   :rows="10" :value="customer1" class="p-datatable-gridlines" dataKey="id"
                   filterDisplay="menu"
                   responsiveLayout="scroll">

          <template #header>
            <div class="flex justify-content-between flex-column sm:flex-row">
              <Button class="p-button-outlined mb-2" icon="pi pi-filter-slash" label="Clear" type="button"
                      @click="clearFilter1()"/>
          ...
          <Column filterField="country.name" header="Country" style="min-width:12rem">
            <template #body="{data}">
              <img :alt="data.country.name" :class="'flag flag-' + data.country.code"
                   src="../assets/demo/flags/flag_placeholder.png" width="30"/>
              <span class="image-text" style="margin-left: .5em; vertical-align: middle">{{ data.country.name }}</span>
            </template>
            <template #filter="{filterModel}">
          ...
          <Column :filterMenuStyle="{'width':'14rem'}" :showFilterMatchModes="false" filterField="representative"
            ...
            <template #body="{data}">
              <img :alt="data.representative.name" :src="'images/avatar/' + data.representative.image" style="vertical-align: middle"
                   width="32"/>
              <span class="image-text"
                    style="margin-left: .5em; vertical-align: middle">{{ data.representative.name }}</span>
            </template>
            <template #filter="{filterModel}">
              ...
              <MultiSelect v-model="filterModel.value" :options="representatives" class="p-column-filter" optionLabel="name"
                ...
                <template #option="slotProps">
                  <div class="p-multiselect-representative-option">
                    <img :alt="slotProps.option.name" :src="'images/avatar/' + slotProps.option.image" style="vertical-align: middle"
                         width="32"/>
                    <span class="image-text"
                          style="margin-left: .5em; vertical-align: middle">{{ slotProps.option.name }}</span>
                  </div>
                </template>
              </MultiSelect>
            </template>
              ...
              <Dropdown v-model="filterModel.value" :options="statuses" :showClear="true" class="p-column-filter"
                ...
                <template #value="slotProps">
                  <span v-if="slotProps.value"
                        :class="'customer-badge status-' + slotProps.value">{{ slotProps.value }}</span>
          ...
          <Column :showFilterMatchModes="false" field="activity" header="Activity" style="min-width:12rem">
            <template #body="{data}">
              <ProgressBar :showValue="false" :value="data.activity" style="height:.5rem"></ProgressBar>
            </template>
            <template #filter={filterModel}>
      ...
      <div class="card">
        ...
        <DataTable :loading="loading2" :scrollable="true" :value="customer2" class="mt-3" scrollDirection="both"
          ...
          <Column :style="{width:'200px'}" field="country.name" header="Country">
            <template #body="{data}">
              <img :class="'flag flag-' + data.country.code" src="../assets/demo/flags/flag_placeholder.png"
                   width="30"/>
              <span class="image-text" style="margin-left: .5em; vertical-align: middle">{{ data.country.name }}</span>
            </template>
          </Column>
    ...
    <div class="col-12">
      <div class="card">
        ...
        <DataTable :sortOrder="1" :value="customer3" groupRowsBy="representative.name"
                   rowGroupMode="subheader" scrollHeight="400px" scrollable sortField="representative.name" sortMode="single">
          <Column field="representative.name" header="Representative"></Column>
          <Column field="name" header="Name" style="min-width:200px"></Column>
          <Column field="country" header="Country" style="min-width:200px">
            <template #body="slotProps">
              <img :class="'flag flag-' + slotProps.data.country.code" src="../assets/demo/flags/flag_placeholder.png"
                   width="30"/>
